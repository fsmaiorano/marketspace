...
public class OrderService(
    ...
    IBusinessLogger<OrderService> businessLogger,
    ...
    : BaseService(httpClient), IOrderService
{
    ...
    public async Task<Result<CreateOrderResponse>> CreateOrderAsync(CreateOrderRequest request)
    {
        ...
        {
            businessLogger.LogInformation("Order created successfully: {@Order}", content);
            return Result<CreateOrderResponse>.Success(content);
        }
        else
        {
            applicationLogger.LogError("Failed to create order. Status code: {StatusCode}, Response: {@Response}",
    ...
    public async Task<Result<GetOrderResponse>> GetOrderByIdAsync(Guid orderId)
    {
        ...
        if (response.IsSuccessStatusCode && content is not null)
        {
            applicationLogger.LogInformation("Order retrieved successfully: {@Order}", content);
            return content;
        }
        else
        {
            applicationLogger.LogError("Failed to retrieve order. Status code: {StatusCode}, Response: {@Response}",
                response.StatusCode, response.Content);
            string errorMessage = await response.Content.ReadAsStringAsync();
    ...
    public async Task<Result<UpdateOrderResponse>> UpdateOrderAsync(UpdateOrderRequest request)
    {
        ...
        if (response.IsSuccessStatusCode && content is not null)
        {
            businessLogger.LogInformation("Order updated successfully: {@Order}", content);
            return content;
        }
        else
        {
            applicationLogger.LogError("Failed to update order. Status code: {StatusCode}, Response: {@Response}",
                response.StatusCode, response.Content);
            string errorMessage = await response.Content.ReadAsStringAsync();
    ...
    public async Task<Result<DeleteOrderResponse>> DeleteOrderAsync(Guid orderId)
    {
        ...
        if (response.IsSuccessStatusCode && content is not null)
        {
            businessLogger.LogInformation("Order deleted successfully: {@Order}", content);
            return content;
        }
        else
        {
            applicationLogger.LogError("Failed to delete order. Status code: {StatusCode}, Response: {@Response}",
                response.StatusCode, response.Content);
            string errorMessage = await response.Content.ReadAsStringAsync();
    ...
    public async Task<Result<GetOrderListResponse>> GetOrdersByCustomerIdAsync(Guid customerId)
    {
        ...
        if (response.IsSuccessStatusCode && content is not null)
        {
            applicationLogger.LogInformation("Orders retrieved successfully for customer: {CustomerId} with {Count} orders", 
                customerId, content.Data?.Orders.Count);
            return content;
        }
        else
        {
            applicationLogger.LogError("Failed to retrieve orders. Status code: {StatusCode}, Response: {@Response}",
                response.StatusCode, response.Content);
            string errorMessage = await response.Content.ReadAsStringAsync();
