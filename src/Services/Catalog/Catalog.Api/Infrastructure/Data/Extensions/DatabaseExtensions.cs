        ...
        try
        {
            ...
            if (context.Database.IsRelational())
            {
                ...
                try
                {
                    ...
                    if (!canConnect)
                    {
                        Console.WriteLine("Catalog: Database does not exist, will be created during migration.");
                    }
                }
                catch (Exception)
                {
                    // Database doesn't exist, it will be created during migration
                    Console.WriteLine("Catalog: Database will be created during migration.");
                }
                ...
                {
                    Console.WriteLine($"Catalog: Applying {pendingMigrations.Count()} pending migration(s)...");
                    ...
                    while (retries > 0)
                    {
                        try
                        {
                            ...
                            Console.WriteLine("Catalog database migration completed successfully.");
                            break;
                        }
                        catch (Exception ex) when (retries > 1)
                        {
                            retries--;
                            Console.WriteLine(
                                $"Catalog migration attempt failed, retrying... ({retries} attempts left). Error: {ex.Message}");
                ...
                {
                    Console.WriteLine("Catalog database is up to date, no migrations needed.");
                }
            }
            else
            {
                ...
                Console.WriteLine("Catalog non-relational database ensured.");
            }
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Catalog database initialization failed: {ex.Message}");
