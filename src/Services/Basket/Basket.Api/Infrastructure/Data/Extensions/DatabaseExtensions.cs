        ...
        try
        {
            ...
            if (context.Database.IsRelational())
            {
                ...
                try
                {
                    ...
                    if (!canConnect)
                    {
                        Console.WriteLine("Basket: Database does not exist, will be created during migration.");
                    }
                }
                catch (Exception)
                {
                    // Database doesn't exist, it will be created during migration
                    Console.WriteLine("Basket: Database will be created during migration.");
                }
                ...
                if (pendingMigrations.Any())
                {
                    ...
                    while (retries > 0)
                    {
                        ...
                        catch (Exception ex) when (retries > 1)
                        {
                            retries--;
                            Console.WriteLine(
                                $"Basket migration attempt failed, retrying... ({retries} attempts left). Error: {ex.Message}");
