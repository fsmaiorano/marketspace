...
public class DeleteBasketHandler(
    ...
    IAppLogger<DeleteBasketHandler> logger)
    : IDeleteBasketHandler
{
    public async Task<Result<DeleteBasketResult>> HandleAsync(DeleteBasketCommand command)
    {
        try
        {
            logger.LogInformation(LogTypeEnum.Application, "Processing delete basket request for user: {Username}", command.Username);
            
            await dataRepository.DeleteCartAsync(command.Username);
            
            logger.LogInformation(LogTypeEnum.Business, "Basket deleted successfully. Username: {Username}", command.Username);
            return Result<DeleteBasketResult>.Success(new DeleteBasketResult(true));
        }
        catch (Exception ex)
        {
            logger.LogError(LogTypeEnum.Exception, ex, "An error occurred while deleting the basket: {Command}", command);
