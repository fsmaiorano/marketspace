...
{
    Console.WriteLine();
    Console.WriteLine("Escolha o modo de simulação:");
    Console.WriteLine("  [1] Direto no banco  – acessa DBs diretamente (lógica interna, dados variados)");
    Console.WriteLine("  [2] Via BFF HTTP     – faz chamadas reais ao seu BFF");
    Console.WriteLine("  [3] Ambos            – executa os dois modos em sequência");
...
async Task RunDirectDbModeAsync(int recordsToCreate)
{
    ...
    Console.WriteLine("Criando schemas...");
    ...
    string             baseUrl)
{
    try
    {
        ...
        var payload = new
        {
            ...
            PaymentMethod = faker.Random.Int(1, 3),          // varia método de pagamento
